...
module.exports = async (targetURL, sourceURL) => {
  ...
  if (sourcePage.status === 404) {
    throw new TypeError(
      `You cannot send a webmention if the post is not published on your side. Failing to send "${sourceURL}" to "${targetURL}"`
    );
  }
  ...
  if (!url) {
    console.warn(
      `Tried to send a webmention to target "${targetURL}" for source "${sourceURL}" but could not find a suitable URL. Skipping`
    );
    return;
  }
